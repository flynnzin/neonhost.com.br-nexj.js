...
export const Footer = () => {
  ...
  return (
    <footer className="relative w-full bg-[#080A0C] border-t border-gray-800/50 overflow-hidden">
      ...
      <div className="relative z-10 w-full mx-auto px-4 sm:px-6 py-12">
              ...
              <div className="flex space-x-4">
                {socialLinks.map((social, index) => (
                  <motion.a
                    ...
                    className={`p-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 transition-all duration-300 ${social.color} hover:border-current hover:bg-white/10`}
              ...
              >
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h2 className="font-medium text-white relative inline-flex items-center">
                    {section.title}
                    <motion.span
                      className="absolute -bottom-1 left-0 h-[2px] bg-gradient-to-r from-purple-500 to-pink-500"
                      initial={{ width: 0 }}
                ...
                <ul className="space-y-3">
                  {section.links.map((link, linkIndex) => (
                    ...
                    >
                      <Link
                        href={link.url}
                        className="group/link flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-all duration-300"
                        target={"external" in link && link.external ? "_blank" : undefined}
                        rel={"external" in link && link.external ? "noopener noreferrer" : undefined}
                      >
                        <motion.span className="relative flex-1" whileHover={{ x: 4 }} transition={{ duration: 0.2 }}>
                          {link.name}
                          <span className="absolute bottom-0 left-0 w-0 h-[1px] bg-gradient-to-r from-purple-500 to-pink-500 transition-all duration-300 group-hover/link:w-full"></span>
                        </motion.span>
                        ...
                        {"badge" in link && link.badge && (
                          <motion.span
                            className="inline-flex items-center rounded-full bg-gradient-to-r from-purple-500/20 to-pink-500/20 px-2 py-0.5 text-xs font-medium text-purple-400 border border-purple-500/30"
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
        ...
        <motion.div
          className="h-[1px] w-full bg-gradient-to-r from-transparent via-gray-800 to-transparent my-8"
          initial={{ scaleX: 0 }}
            ...
            <div className="flex gap-6">
              {[
                ...
                <Link
                  key={index}
                  href={item.url}
                  className="text-sm text-gray-500 hover:text-gray-300 transition-colors relative group"
                >
                  {item.name}
