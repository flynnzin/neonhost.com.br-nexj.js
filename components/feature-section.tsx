...
export function FeatureSection({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        {hasChart && (
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 overflow-hidden">
            <div className="aspect-[4/3] relative">
              ...
              <div className="absolute inset-0 grid grid-cols-12 grid-rows-6 gap-1">
                ...
                {Array.from({ length: 7 }).map((_, i) => (
                  <div
                    key={`h-${i}`}
                    className="absolute left-0 right-0 border-t border-gray-200/10"
                    style={{ top: `${(i * 100) / 6}%` }}
                  ></div>
                ))}
                {Array.from({ length: 13 }).map((_, i) => (
                  <div
                    key={`v-${i}`}
                    className="absolute top-0 bottom-0 border-l border-gray-200/10"
                    style={{ left: `${(i * 100) / 12}%` }}
                  ></div>
                ))}

                {/* Linha de proteção */}
                <div className="absolute top-0 bottom-0 border-l-2 border-blue-500/70 left-[80%]"></div>
        ...
        {hasIpComparison && (
          <div className="bg-[#111827] backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden">
            <div className="p-4 bg-[#1f2937] text-center text-gray-400 text-sm">Antes:</div>
            <div className="p-6 text-center font-mono text-lg text-gray-300">192.168.1.254:30120</div>

            <div className="p-4 bg-gradient-to-r from-purple-900/50 to-pink-900/50 text-center text-purple-300 text-sm">
              Depois:
            </div>
        ...
        {!hasChart && !hasIpComparison && (
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm">{description.split(".")[0]}.</p>
            </div>
          </div>
        )}
      </div>
